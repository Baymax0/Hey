...
-(void)layoutSubviews{
    ...
    for (int i = 0; i < self.nodeArray.count; ++i) {
        ...
        CGFloat width = min / 4.3;
        CGFloat height = min / 4.2;
        CGFloat bank = (min-width*3)/2;
        int row = i % 3;
        int column = i / 3;
        CGRect frame = CGRectMake(row *(width +bank), column * (width +bank), width, height);
...
-(void)cleanNodes
{
    ...
    self.polygonalLineLayer.strokeColor = SelectColor.CGColor;
    self.polygonalLineLayer.path = self.polygonalLinePath.CGPath;
}
...
-(CAShapeLayer *)polygonalLineLayer{
    if (_polygonalLineLayer == nil) {
        ...
        _polygonalLineLayer.strokeColor = SelectColor.CGColor;
