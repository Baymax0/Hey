...
open class BubbleTabBarController: UITabBarController {
    ...
    open override func viewDidLoad() {
        ...
        if hideLine == true{
            let v = UIView(frame: CGRect(x: 0, y: -1, width: KScreenWidth, height: 4))
            v.backgroundColor = .KBGGray
    ...
    open override func tabBar(_ tabBar: UITabBar, didSelect item: UITabBarItem) {
        ...
        if let willShowVC = viewControllers?[idx] {
            ...
            selectedIndex = idx
            delegate?.tabBarController?(self, didSelect: willShowVC)
        }
    }
